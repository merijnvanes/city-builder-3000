...
import bpy, sys, os, math, json, argparse, time
...
models={name:(getattr(services,name),3) for name in ['fire','police','hospital','school','jail']}
try:
    import culture, sanitation
    models.update({name:(getattr(culture,name),4 if name=='college' else 2 if name=='library' else 3) for name in ['college','library','museum']})
    models.update({name:(getattr(sanitation,name),1 if name=='landfill' else 3) for name in ['landfill','incinerator','recycling','wasteenergy']})
...
for kind in args.types.split(','):
    ...
    size=int(tiles*64*args.scale+112);scene.render.resolution_x=size;scene.render.resolution_y=size
    ...
    target=Vector((0,0,2.3 if tiles<4 else 2.8));radius=25
    ...
    if args.save_blend:bpy.ops.wm.save_as_mainfile(filepath=str(root/'source-art/civic'/f'{kind}.blend'),compress=True)
