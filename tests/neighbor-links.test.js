...
test('an unpurchased border road remains a dead end and quotes do not mutate the city',()=>{
 ...
 assert.ok(result.ok);assert.equal(c._connections.west.road,0);
...
test('buying a connection charges once, marks the route, survives save and can be undone',()=>{
 ...
 assert.equal(c.money,before-CONNECTION_FEES.road);assert.equal(c._connections.west.road,1);
 ...
 const saved=deserialize(serialize(c));assert.equal(saved._connections.west.road,1);
 undo.undo(c);assert.equal(c._connections.west.road,0);assert.equal(c.money,before);
});
test('each county and transport layer needs its own connection and stale offers fail',()=>{
 ...
 const northRail=offers.find(o=>o.side==='north'&&o.route==='rail');connectNeighbor(c,northRail);
 assert.equal(c._connections.north.rail,1);assert.equal(c._connections.west.rail,0);assert.equal(c._connections.north.road,0);
 place(c,0,0,'bulldoze');assert.equal(c._connections.north.rail,1);
 place(c,0,0,'bulldoze');assert.equal(c.transportConnections.length,0);
 assert.equal(connectNeighbor(c,northRail).ok,false);
 place(c,0,0,'rail');assert.equal(c._connections.north.rail,0);
});
test('the county stays connected until its last purchased route is removed',()=>{
 const c=town();for(const y of [20,21]){place(c,0,y,'road');connectNeighbor(c,{x:0,y,side:'west',route:'road'});}
 place(c,0,20,'bulldoze');assert.equal(c._connections.west.road,1);
 place(c,0,21,'bulldoze');assert.equal(c._connections.west.road,0);
});
test('unaffordable, malformed and interior connections cannot charge money',()=>{
 const c=town();place(c,0,20,'road');c.money=1;
 for(const link of [{x:0,y:20,side:'west',route:'road'},{x:0,y:20,side:'north',route:'road'},{x:1,y:20,side:'west',route:'road'},{x:0,y:20,side:'west',route:'__proto__'}])assert.equal(connectNeighbor(c,link).ok,false);
...
test('legacy saves retain old links; explicit empty lists stay disconnected and corrupt links are rejected',()=>{
 ...
 assert.equal(deserialize(JSON.stringify(raw))._connections.west.road,0);
 delete raw.transportConnections;assert.equal(deserialize(JSON.stringify(raw))._connections.west.road,1);
 raw.transportConnections=[{x:1,y:20,side:'west',route:'road'}];assert.throws(()=>deserialize(JSON.stringify(raw)),/county connection/);
});
test('border-parallel strokes do not prompt for every tile, and existing endpoints still prompt',()=>{
 ...
 assert.equal(approach.connectionOffers.length,1);assert.equal(approach.connectionOffers[0].side,'west');
});
test('two purchased crossing layers supply separate jobs and lower-road customers',()=>{
 ...
 connectNeighbor(c,{x:0,y:20,side:'west',route:'road'});
 ...
 connectNeighbor(c,{x:0,y:20,side:'west',route:'highway'});
 assert.equal(c._connections.west.road,2);assert.equal(c._traffic.externalJobs,one*2);
 assert.equal(c._connections.west.roadLinks.length,2);
});
