...
try {
  ...
  assert.equal(initial.types, 40);
  assert.equal(initial.requests, 40, 'one load per gameplay type');
  ...
  console.log('Working set: 40 types, no repaint reloads, reduced alpha picking, full close-ups, failed upgrades and late-result accounting and portrait history pass.', initial);
