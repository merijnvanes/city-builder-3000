...
try {
 ...
 let hiddenTotal=0,hiddenBuildings=0;
 for(let rotation=0;rotation<4;rotation++) {
  const result=await page.evaluate(async rotation=>{
   const {preloadCivicSprites}=await import('/src/building-art.js');
   const {foundationEdges,occludeLotFoundation,hitFoundation}=await import('/src/lot-foundations.js');
   ...
   // Test the same far-end wall pixels on an isolated solid layer. A rear
   // object must disappear throughout the wall, including beside its ends.
   ...
   base.fillStyle='#ff00ff';base.fillRect(0,0,1000,800);
   occludeLotFoundation(proxy,t);
   let checked=0;
   for(const wall of walls)for(const f of [.1,.5,.9]) {
    const [ax,ay,az]=wall.a,[bx,by,bz]=wall.b,z=az+(bz-az)*f;
    if(wall.top-z<4)continue;
    ...
    if(base.getImageData(Math.round(p.x),Math.round(p.y),1,1).data[3]>8)throw Error(`Unmasked retaining wall in view ${rotation}`);
    const pixel=r.ground.getContext('2d').getImageData(Math.round(p.x*r.dpr),Math.round(p.y*r.dpr),1,1).data;
    if(wall.facing && pixel[1]>pixel[0]+2)throw Error(`Terrain overwrites the support at view ${rotation}, segment ${f}: ${Array.from(pixel)}`);
    checked++;
   }
   ...
   for(let i=0;i<expected.length;i+=4)if(expected[i+3]===255) {
    treePixels++;
    if(actual[i+3]<253 || Math.max(...[0,1,2].map(c=>Math.abs(expected[i+c]-actual[i+c])))>2)covered++;
    if(i%80===0) {
   ...
   for(const item of r.items.slice(0,r.items.findIndex(item=>item.t===t)))if(item.kind==='trees') {
    ...
    for(let i=0;i<rear.length;i+=4)if(rear[i+3]===255 && actual[i+3]===0) {
     const x=(i/4)%1000+.5,y=Math.floor(i/4/1000)+.5;
     if(hitFoundation(foundation.polygons,x,y)) {
  ...
  hiddenTotal+=result.hidden;hiddenBuildings+=result.hiddenBuilding;
  console.log(result);
  await page.screenshot({path:`artifacts/scene-depth-${rotation}.png`});
 }
 assert.ok(hiddenTotal>0,'a real rear tree must be erased from the final solid layer');
 assert.ok(hiddenBuildings>0,'erased pixels from a real cached building must select the foundation');
  ...
  for(let rotation=0;rotation<4;rotation++) {
   ...
   for(const [x,y] of [[13,13],[17,13],[13,17],[17,17]]) {
    ...
    for(let i=0;i<expected.length;i+=4)if(expected[i+3]===255) {
     ...
     if(!visible)covered++;
     // For a rear tree, only an erased pixel proves occlusion. Similar
     // greens in the building artwork can otherwise mimic a visible leaf.
     if(i%80===0 && (!behind || actual[i+3]===0)) {
      const pick=r.pickObject((i/4)%1000+.5,Math.floor(i/4/1000)+.5);
 ...
 console.log('Closed terraced corners, rear-object wall masks and foreground tree pixels pass in all four views.');
