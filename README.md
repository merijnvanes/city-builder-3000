...
- **Neighbors**: roads, highways and rails to the map edge connect you to a named neighboring city for trade and garbage deals. Power lines and pipes enable power and water deals. Currently only road/highway connections add outside jobs; this restriction is not verified as a SimCity 3000 rule (see the source notes in `src/sim/traffic.js`).
